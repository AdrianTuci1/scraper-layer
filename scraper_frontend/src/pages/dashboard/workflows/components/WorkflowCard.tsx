...
} from '@/pages/workflow/runs/[workflowId]/_components/ExecutionStatusIndicator';
    ...
    <Card className="border border-separate shadow-sm rounded-lg overflow-hidden hover:shadow-md dark:shadow-primary/30 group/card">
      <CardContent className="p-4 flex items-center justify-between h-[100px]">
          ...
          <div>
            <h3 className="text-base font-bold text-muted-foreground flex items-center">
              <TooltipWrapper content={workflow.description || ''}>
                <Link to={`/dashboard/workflow/editor/${workflow.id}`}>
                  {workflow.name}
                </Link>
              </TooltipWrapper>
              {isDraft && (
            ...
            <SchedulerSection
              ...
              workflowId={workflow.id}
              workflowCron={workflow.cron ?? null}
            />
          </div>
        </div>
        <div className="flex items-center space-x-2">
          ...
          <Link
            to={`/dashboard/workflow/editor/${workflow.id}`}
            className={cn(
...
function LastRunDetails({ workflow }: { workflow: Workflow }) {
  ...
  const nextScheduleUtc =
    nextRunAt && format(new Date(nextRunAt), 'HH:mm') + ' UTC';
  ...
  return (
    <div className="bg-primary/5 px-4 py-1 flex justify-between items-center text-muted-foreground">
      <div className="flex items-center text-sm gap-2">
        {lastRunAt && (
          <Link
            to={`/dashboard/workflow/runs/${workflow.id}/${lastRunId}`}
