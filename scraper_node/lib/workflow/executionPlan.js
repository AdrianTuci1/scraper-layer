const { FlowToExecutionPlanValidationError, TaskParamType } = require('./constants');
  ...
  for (
    ...
    for (const currentNode of nodes) {
      ...
      if (invalidInputs?.length > 0) {
        // If has invalid inputs i.e. Node input is blank, then checking for if node has input from previous node
        const incomers = getIncomers(currentNode, nodes, edges);

        // Checking if the incomer node is already planned?
        if (incomers.every((incomer) => planned.has(incomer.id))) {
          // If this hits it means all the incoming nodes are planned and thus the current-node has invalid inputs
...
function getInvalidInputs(node, edges, planned) {
  ...
  for (const input of inputs) {
    ...
    // If value is not provided by user, then check if there is output link i.e. this input incoming data from other node

    // Checking all the incoming connections to the node
    const incomingEdges = edges.filter((edge) => edge.target === node.id);

    // Checking if input of the incoming node is connected to the current input
